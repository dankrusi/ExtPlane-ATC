    ...
    QSettings(organization, application, parent)
{
    ...
    foreach(QString arg, parsedArguments.keys()) {
        DEBUG << "detected command line argument" << arg << "with value" << parsedArguments.value(arg);
    }
}
...
QString Settings::getLocalDir(QString dirName)
{
    return valueFromSettingsOrCommandLine("app-dir",Util::GetLocalAppDir(dirName)).toString();
}

QString Settings::getLocalFile(QString dirName, QString fileName)
{
    QString filePath = getLocalDir(dirName) + QDir::separator() + fileName;
    return filePath;
}
